...
def enhance_view(request):
    ...
    try:
        ...
        model_arg = preferred_model if preferred_model in ('gemini', 'groq', 'nvidia', 'mistral') else None
...
def analyze_url_view(request):
    ...
    try:
        ...
        model_arg = data.get('model') if data.get('model') in ('gemini', 'groq', 'nvidia', 'mistral') else None
        api_key = request.headers.get('X-API-Key')
