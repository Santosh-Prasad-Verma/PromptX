...
def enhance_view(request):
    ...
    try:
        ...
        model_arg = preferred_model if preferred_model in ('gemini', 'groq', 'nvidia', 'mistral', 'llama_405b', 'glm', 'deepseek') else None
...
def analyze_url_view(request):
    ...
    try:
        ...
        model_arg = data.get('model') if data.get('model') in ('gemini', 'groq', 'nvidia', 'mistral', 'llama_405b', 'glm', 'deepseek') else None
        api_key = request.headers.get('X-API-Key')
